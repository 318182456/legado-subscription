...
export async function drawTheme(ctx: CanvasRenderingContext2D, cfg: any, options: RenderOptions) {
    ...
    const toRgba = (color: any, alpha?: number): string => {
        ...
        if (typeof color !== 'string') return `rgba(0,0,0,${alpha ?? 1})`;
        if (!color.startsWith('#')) return color;

        let r = 0, g = 0, b = 0, a = alpha ?? 1;
        if (color.length === 9) { // #AARRGGBB
            a = alpha ?? (parseInt(color.slice(1, 3), 16) / 255);
    ...
    const lineSpacingMultiplier = (cfg.lineSpacingExtra ?? 12) / 10;
    const paraSpacingMultiplier = (cfg.paragraphSpacing ?? 5) / 10;

    const lineHeight = fontSize * lineSpacingMultiplier;
    const paraSpacing = fontSize * paraSpacingMultiplier;

    const textColor = toRgba(cfg.textColor ?? "#ff43050a");
    const tipColor = toRgba(cfg.tipColor ?? "#ff4d3838");

    const pL = (cfg.paddingLeft ?? 22) * d;
    const pR = (cfg.paddingRight ?? 22) * d;
    const contentW = W - pL - pR;
    
    const measure = (str: string, font?: string): number => {
        const old = ctx.font;
    ...
    const paragraphIndent = cfg.paragraphIndent ?? "　　";
    const indentW = measure(paragraphIndent) + (paragraphIndent.length - 1) * getLetterSpacing(fontSize, letterSpacingEm);

    // ── 2. 分词与测量引擎 ─────────────────────────────────
    const segmenter = typeof Intl !== "undefined" && Intl.Segmenter ? new Intl.Segmenter("zh-CN", { granularity: "word" }) : null;

    const layoutLines = (text: string, maxW: number, firstIndent: number, curFontSize: number = fontSize): string[][] => {
        let tokens: string[] = [];
        if (segmenter) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            const currentFont = `${isBold}${curFontSize}px "${fontName}", sans-serif`;
    ...
    const drawLineBaseline = (tokens: string[], x: number, yBaseline: number, align: "left" | "justify", targetWidth: number, curFontSize: number) => {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            sx += ws[i] + (isLast ? 0 : currentLetterSp) + extraSpacePerGap + (i < remainGap ? 1 : 0) + (tokens[i].trim() === "" ? extraSpacePerSpaceChar : 0);
        }
    };
    ...
    if (cfg.headerMode !== 2) {
        ...
        ctx.font = `${hFontSize}px sans-serif`;
        ctx.fillStyle = tipColor;
        ...
        ctx.fillText(rightText, W - (cfg.headerPaddingRight ?? 22) * d - ctx.measureText(rightText).width, hBaseY);
    ...
    // 标题区域
    if (cfg.titleMode !== 2) {
        const tTitleSize = (cfg.titleSize ?? 4);
        const tFontSize = ((cfg.textSize ?? 20) + tTitleSize) * d;
        const tMetrics = getRealFontMetrics(ctx, tFontSize);
        const tLineHeight = tFontSize * lineSpacingMultiplier;
        const tParaSpacing = tFontSize * paraSpacingMultiplier;

        ctx.font = `bold ${tFontSize}px "${fontName}", sans-serif`;
        ctx.fillStyle = textColor;
        currentY += (cfg.titleTopSpacing ?? 0) * d;

        const tLines = layoutLines(cleanTitle, contentW, 0, tFontSize);
        for (let i = 0; i < tLines.length; i++) {
            const tBaseline = currentY + tMetrics.ascent;
            drawLineBaseline(tLines[i], pL, tBaseline, "left", contentW, tFontSize);
            currentY += tLineHeight;
            if (i === tLines.length - 1) currentY += tParaSpacing;
        }
        currentY += (cfg.titleBottomSpacing ?? 0) * d;
    }
    ...
    const footerSafeH = cfg.footerMode !== 1 ? (cfg.footerPaddingBottom ?? 9) * d + fFontSize + (cfg.footerPaddingTop ?? 0) * d + navH : navH;
    ...
    outer: for (let pi = 0; pi < cleanParas.length; pi++) {
        ...
        for (let li = 0; li < lines.length; li++) {
            ...
            const bodyBaseline = currentY + ascent;

            drawLineBaseline(lines[li], currentX, bodyBaseline, justifyMode, targetW, fontSize);
            currentY += lineHeight;
        }
        if (pi < cleanParas.length - 1) currentY += paraSpacing;
    }
